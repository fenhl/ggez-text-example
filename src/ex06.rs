...
impl TextBox {
    ...
    fn draw(mut self, handler: &crate::Handler, ctx: &mut Context) -> GameResult {
        ...
        self.text
            ...
            .set_bounds([w - self.size, h - self.size], self.halign)
...
fn main() -> GameResult {
    let (mut ctx, evt_loop) = ContextBuilder::new("ex06", "Fenhl")
        .window_setup(WindowSetup {
            title: format!("ggez 0.6 example"),
            ..WindowSetup::default()
        })
    ...
    ggez06::event::run(ctx, evt_loop, handler)
}
